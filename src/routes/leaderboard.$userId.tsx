...
function MatchCard({ match }: { match: BracketMatch }) {
  const row = (team: string | null, label: string) => {
    ...
    return (
      <div
        className={[
          "w-full text-left px-3 py-2.5 text-xs sm:text-sm rounded-md border uppercase tracking-wide font-semibold flex items-center justify-between gap-2",
          selected
